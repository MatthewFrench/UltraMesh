...
class Input {
  public:
    ...
    static void poll() {
      while (Event.is_happening) {
        switch (Event.type) {
          case KeyDown:
            if (Event.scancode in keyDown) {
						keyDown[Event.scancode]();
            }
            break;
				case KeyUp:
					if (Event.scancode in keyUp) {
	...
	static void key_up(in int key, in void delegate() callback) {
		keyUp[key] = callback;
	}
  ...
  private:
    static void delegate()[int] keyDown;
	static void delegate()[int] keyUp;
