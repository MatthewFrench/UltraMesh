...
struct CoordinateSystem {
  public:
    ...
    void set_position(Vec3 x, Vec3 y, Vec3 z) {
		_list = [ Quat(1.0,x), Quat(1.0,y), Quat(1.0,z) ];
    }
    ...
    @property {
      ...
      Vec3 z() {
			//return _list[2].vec3;return null;
			return Vec3(0,0,0);
      }
    }

  private:
    Quat[] _list = [ Quat(0.0,Vec3(1, 0, 0)),
		Quat(0.0,Vec3(0, 1, 0)),
		Quat(0.0,Vec3(0, 0, 1)) ];
}
...
class Geometry {
  public:
    ...
    void pitch(in float rad) {
      //r//otate_impl(_localCS.x, rad, _origin.vec3);
    }
	...
	float[] position;
	float[] getVerticeCoordinates() {
		if (geometryChanged) {
			writeln("Geometry recalculating");
			geometryChanged = false;
			position = [];
			foreach (vec3; vertices) {
				  //position ~= vec3.coord;
			}
		}
		return position;
	...
	float[] color;
	float[] getVerticeColors(Material material, int ColorMax) {
		if (colorChanged) {
			writeln("Color recalculating");
			colorChanged = false;
		// VBO: Color
		...
		color = colorBase.cycle.take(colorBase.length * vertices.length).array;
		}
		return color;
	}
	bool iboChanged = true;
	IBO ibo;
	IBO getIBO() {
		if (iboChanged) {
			writeln("IBO recalculating");
			iboChanged = false;
		ibo = new IBO();
		ibo.create(indices);
		}
	...
	VBO positionVBO;
	VBO colorVBO;
	bool vbosChanged = true;
	void setVBOs(ShaderProgram program) {
		if (vbosChanged) {
			writeln("VBOs recalculating");
			vbosChanged = false;
			positionVBO = new VBO;
			colorVBO = new VBO;
			positionVBO.set(program, position, "position", 3, 0);
			colorVBO.set(program, color, "color", 4, 1);
		}
	}
  ...
  protected:
    ...
    Quat _origin = Quat(0.0,Vec3(0, 0, 0));
    Vec3[] _vertices;
    int[] _indices;
    Vec3[] _normals;
}
